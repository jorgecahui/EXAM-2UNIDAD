...
class ClientService {
  ...
  Future<(SyncResult, int?)> save(Client client) async {
    ...
    if (response.statusCode == 400) {
      final serverId = await _findServerIdByDoc(client.documentNumber);
      return (SyncResult.duplicate, serverId);
    }
    return (SyncResult.error, null);
  }

  Future<int?> _findServerIdByDoc(String documentNumber) async {
    try {
      var url = Uri.http(apiUrl, '/client/clients/', {'document_number': documentNumber});
